...
/**
 * Implementation of a Bloom-filter: http://en.wikipedia.org/wiki/Bloom_filter
 *
 * @author Pawel Perzan
...
public class BloomFilter<E> implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 135427841449071663L;
	...
	public BloomFilter(int m, int n, int k) {
		...
		this.bitSetSize = m; 				// (int)Math.ceil(c * n);
		numberOfAddedElements = 0;
		this.bitset = new BitSet(bitSetSize);
	}
	...
	public BloomFilter(double falsePositiveProbability, int expectedNumberOfElements) {
		this(Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2))) / Math.log(2), 		// c = k / ln(2)        (taken from wiki: c is a constant, c = m/n, so: m = c * n)
